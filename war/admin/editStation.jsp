...
<%
	User sessionUser = (User)session.getAttribute("user");
  	...
  	Station station = StationManager.getStation(KeyFactory.stringToKey(request.getParameter("k")));
...
<form method="post" id="form1" name="form1" 
    ...
    <fieldset>
	<legend><%=printer.print("Station")%></legend>
	...
	<div>
        ...
        <select name="s_type" <%=readOnly ? "disabled=\"true\"" : ""%>>
        <%
	...
	<div>
		...
		<select name="s_privilege" <%=readOnly ? "disabled=\"true\"" : ""%>>
        	<option class="s_privilege" value="0" <%=station.getStationPrivilegeLevel() == 0 ? "selected=\"true\"" : ""%>><%= printer.print("Basic") %></option>
        	<option class="s_privilege" value="1" <%=station.getStationPrivilegeLevel() == 1 ? "selected=\"true\"" : ""%>><%= printer.print("Pro") %></option>
        </select>
	...
	%>
		<div>
	        ...
	        <a href="../station/editStationPassword.jsp?s_key=<%= request.getParameter("k") %>"><%= printer.print("Change password") %></a>
		</div>
	<%
	}
	%>
	...
	<div>
		<h2>
			<a href="../station/listChannel.jsp?s_key=<%= request.getParameter("k") %>"><%= printer.print("Channels") %></a>
		</h2>
	</div>
